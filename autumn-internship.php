...
function view_autumn_intern_card_func(){
    $item_type = "autumn_internship";
    $args = array(
...
add_shortcode('view-autumn-intern-card','view_autumn_intern_card_func');

function template_autumn_internship2_func($content){
    global $post;
    $post_id=$post->ID;
    ...
    if($entry_url == ""){
      $entry_html = '
        ...
        $entry_button = '
        <a href="[get_form_address formtype=apply form_id=4767 post_id='.$post->ID.' title='.$post_title.']">
          <button class="button detail">プレエントリー</button>
        </a>';
	  }
  	else{
      $entry_html = '
        <a href="[get_form_address formtype=apply form_id=6840 post_id='.$post->ID.' title='.$post_title.' redirect ='.$entry_url.']">
            <button class="button button-apply">プレエントリーする</button>
        </a>';
      $entry_button = '
      <a href="[get_form_address formtype=apply form_id=6840 post_id='.$post->ID.' title='.$post_title.' redirect ='.$entry_url.']">
